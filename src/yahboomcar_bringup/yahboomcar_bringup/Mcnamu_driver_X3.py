...
class yahboomcar_driver(Node):
	def __init__(self, name):
		...
		global car_type_dic
		self.RA2DE = 180 / pi
		# Resolve serial device before touching hardware (Rosmaster ctor opens the port).
		self.declare_parameter('serial_port', '/dev/myserial')
		_serial = self.get_parameter('serial_port').get_parameter_value().string_value
		self.get_logger().info('Rosmaster serial port: %s' % _serial)
		self.car = Rosmaster(car_type=1, com=_serial)
		...
		self.cmd_watchdog_triggered = False

		# Mechanical-asymmetry trim. Counteracts the fact that this X3 unit
		# physically curves to one side when commanded pure forward motion
		# (per-wheel encoder rates differ by a few percent across the four
		# motors; see odom_drift_checker / wheel_balance_diagnostic findings).
		#
		# trim_vy_per_vx : when commanding linear.x, add this fraction of vx
		#                  to vy. Positive value pushes the chassis to its
		#                  left; if your robot drifts right when commanded
		#                  forward, set this positive.
		# trim_w_per_vx  : when commanding linear.x, add this fraction of vx
		#                  to angular.z (rad/s per m/s). Compensates for any
		#                  residual yaw bias in forward motion.
		#
		# Both default to 0.0 so the driver behaves unchanged unless you
		# explicitly calibrate. Tune by driving 1 m forward and inspecting
		# odom_drift_checker's reported delta y / delta yaw, then iterate.
		self.declare_parameter('trim_vy_per_vx', 0.0)
		self.trim_vy_per_vx = self.get_parameter('trim_vy_per_vx').get_parameter_value().double_value
		self.declare_parameter('trim_w_per_vx', 0.0)
		self.trim_w_per_vx = self.get_parameter('trim_w_per_vx').get_parameter_value().double_value
		self.get_logger().info(
			'cmd_vel trim: trim_vy_per_vx=%.4f, trim_w_per_vx=%.4f' % (
				self.trim_vy_per_vx, self.trim_w_per_vx))
	...
	def cmd_vel_callback(self,msg):
		...
		# Apply mechanical-asymmetry trim before sending to the firmware.
		# The firmware's per-wheel PID will then implement the corrected
		# (vx, vy_corrected, w_corrected) command, which on this unit
		# happens to come out as something close to (vx, 0, 0) physically.
		vy_corrected = vy + self.trim_vy_per_vx * vx
		w_corrected  = angular + self.trim_w_per_vx * vx

		self.car.set_car_motion(vx, vy_corrected, w_corrected)
		'''print("cmd_vx: ",vx)
		print("cmd_vy: ",vy)
	...
	#pub data
	def pub_data(self):
		# Safety watchdog: if command stream stalls (node crash/disconnect),
		# force the chassis to stop instead of keeping the last motion command.
		elapsed = time.monotonic() - self.last_cmd_time
		if elapsed > self.cmd_timeout_sec:
			if not self.cmd_watchdog_triggered:
				self.get_logger().warn(
					f"cmd_vel timeout {elapsed:.2f}s > {self.cmd_timeout_sec:.2f}s, forcing stop")
				self.cmd_watchdog_triggered = True
...
def main():
	...
	try:
		...
		# Final safety stop on Ctrl+C/shutdown.
